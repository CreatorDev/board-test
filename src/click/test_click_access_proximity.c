...
static int proximity_read(int fd, uint32_t *prox)
{
    ...
    if (read_register(fd, PROX_RESULT_LOW_REG, &buf[0]) < 0) {
        printf("I2C error: failed to read proximity result low\n");
        return -1;
    }

    if (read_register(fd, PROX_RESULT_HIGH_REG, &buf[1]) < 0) {
        printf("I2C error: failed to read proximity result high\n");
        return -1;
    }
...
int main(int argc, char *argv[])
{
    ...
    case AT_ENABLE:
        ...
        printf("Proximity periodic measurements have been enabled\n");
        break;
    case AT_DISABLE:
        ...
        printf("Periodic measurements have been disabled\n");
        break;
